...
import {Container, Form, Input, Box, Text,Button, H1, Title, NavLink, BoxLinks, Error, TitlePage} from './SignupStyle'
function Signup() {
    // const{code} = useSelector((state)=>state.signin)
    // console.log(code)
    ...
    //         body: JSON.stringify({"username": Username, "name": Name, "country": Country, "city": City, "phone": Phone, "email":Email, "password": Pass})
    //     };
    ...
    const handleClick = (x) =>{
        ...
        if (accept1 == true && accept2 == true && accept3 == true && accept4 == true && accept5 == true && accept6 == true && accept7 == true ) {
            ...
            // dispatch(enterUser({Name, Username, Pass, Country, City, Email, Phone}))
            dispatch(enterUser({"username": Username, "name": Name, "country": Country, "city": City, "phone": Phone, "email":Email, "password": Pass}))
            setStyle({
                display: 'block'
            })
